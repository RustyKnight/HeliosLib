...
//    }
     ...
     */
    public ZonedDateTime computeSunriseCalendar(Zenith solarZenith, ZonedDateTime date) {
        return getLocalTimeAsZonedDateTime(computeSolarEventTime(solarZenith, date, true), date);
    }
     ...
     */
    public ZonedDateTime computeSunsetCalendar(Zenith solarZenith, ZonedDateTime date) {
        return getLocalTimeAsZonedDateTime(computeSolarEventTime(solarZenith, date, false), date);
    }

    private BigDecimal computeSolarEventTime(Zenith solarZenith, ZonedDateTime date, boolean isSunrise) {
    ...
    protected ZonedDateTime getLocalTimeAsZonedDateTime(BigDecimal localTimeParam, ZonedDateTime date) {
...
//            resultTime.add(Calendar.HOUR_OF_DAY, -24);
        ...
        ZonedDateTime result = ZonedDateTime.of(date.getYear(), 
                date.getMonthValue(), 
                date.getDayOfMonth(), 
                hour, 
                minutes.intValue(), 
...
//        resultTime.setTimeZone(date.getTimeZone());
    ...
    private BigDecimal getLocalTime(BigDecimal localMeanTime, ZonedDateTime date) {
    ...
    private BigDecimal adjustForDST(BigDecimal localMeanTime, ZonedDateTime date) {
        BigDecimal localTime = localMeanTime;    
        //System.out.println("isDayLightSavings = " + date.getZone().getRules().isDaylightSavings(date.toInstant()));
        //if (date.getZone().getRules().isDaylightSavings(date.toInstant())) {
...
//     *
//     * @param localTime <code>BigDecimal</code> representation of the local
//     * rise/set time.
//     * @return <code>String</code> representation of the local rise/set time in
//     * HH:MM format.
//     */
...
//    }
     ...
     * Returns the local rise/set time in the form HH:MM.
     *
     * @param localTimeParam <code>BigDecimal</code> representation of the local
     * rise/set time.
     * @return <code>Calendar</code> representation of the local time as a
     * calendar, or null for none.
     */
     ...
     * *****************
     */
    private BigDecimal getDayOfYear(ZonedDateTime date) {
        return new BigDecimal(date.getDayOfYear());
    }

    private BigDecimal getUTCOffSet(ZonedDateTime date) {
        BigDecimal offSetInMillis = new BigDecimal(date.getOffset().getRules().getOffset(date.toInstant()).getTotalSeconds() * 1000);
